...
use crate::ui::CrosstermAppBackend;
...
pub fn load_preview_crossterm(context: &mut AppContext, backend: &mut CrosstermAppBackend) {
    let mut load_list = Vec::with_capacity(2);
    ...
    for (path, metadata) in load_list {
        load_preview_path_crossterm(context, backend, path, metadata);
    }
}

pub fn load_preview_path_crossterm(
    context: &mut AppContext,
    ...
    if metadata.is_dir() {
        ...
        if need_to_load {
            preview_file::Background::preview_path_with_script_crossterm(context, backend, p);
        }
    } else {
    }
}
